...
public class RouteController implements RouteApi {
    ...
    public BaseResponse<CIMServerResVO> login(@RequestBody LoginReqVO loginReqVO) throws Exception {
        ...
        // 用户不存在 注册一个
        if (StatusEnum.ACCOUNT_NOT_MATCH == status) {

            RegisterInfoResVO registerInfoResVO = new RegisterInfoResVO();
            registerInfoResVO.setUserId(loginReqVO.getUserId());
            registerInfoResVO.setUserName(loginReqVO.getUserName());
            accountService.register(registerInfoResVO);
